...
- `.pluck`: Select specific columns to plot (default: FALSE shows all)
  - Use exact plotted column names such as `"y"`, `"cum_sum_y"`, `"cum_sum_x"`, or `"cum_sum_z"`
  - Short cumulative aliases such as `"cum_sum"` work only when they match exactly one plotted column
  - Can be a vector: `c("y", "cum_sum_y")`

#' Generate Multiple Random Beta Walks in Multiple Dimensions (Rcpp Version)
#'
...
#'
#' @description
#' The `random_beta_walk_rcpp` function is an Rcpp-optimized version of 
#' `random_beta_walk` that generates multiple random walks in 1, 2, or 3 dimensions.
...
#'
#' This version uses C++ implementations for cumulative statistics computation, which can
#' provide performance improvements, especially for larger datasets.
#'
...
#'
#' set.seed(123)
#' random_beta_walk_rcpp(.dimensions = 3, .shape1 = c(2, 3, 4), .shape2 = c(2, 3, 4), .ncp = c(0, 1, 2)) |>
#'   head() |>
#'   t()
#' }
#'
#' @export
#' @rdname random_beta_walk_rcpp
random_beta_walk_rcpp <- function(
  .num_walks = 25,
  .n = 100,
  .shape1 = 2,
  .shape2 = 2,
  ...
  # Use Rcpp for cumulative statistics - more efficient for larger datasets
  # Process each dimension separately using Rcpp batch operations
  res_list <- list(res)
  
  for (col in dim_names) {
    # Split by walk_number and apply Rcpp batch cumulative stats
    res <- res |>
      dplyr::group_by(walk_number) |>
      dplyr::group_modify(~ {
        col_data <- .x[[col]]
        # Use Rcpp batch function for all cumulative stats at once
        cum_stats <- rcpp_batch_cumstats(col_data, initial_value)
        .x[[paste0("cum_sum_", col)]] <- cum_stats[, 1]
        .x[[paste0("cum_prod_", col)]] <- cum_stats[, 2]
        .x[[paste0("cum_min_", col)]] <- cum_stats[, 3]
        .x[[paste0("cum_max_", col)]] <- cum_stats[, 4]
        .x[[paste0("cum_mean_", col)]] <- cum_stats[, 5]
        .x
      }) |>
      dplyr::ungroup() |>
      dplyr::as_tibble()
  ...
  attr(res, "fns")           <- "random_beta_walk_rcpp"

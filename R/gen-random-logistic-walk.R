...
#'
#' @description
#' The `random_logistic_walk` function generates multiple random walks in 1, 2, or 3 dimensions.
#' Each walk is a sequence of steps where each step is a random draw from a logistic distribution.
#' The user can specify the number of walks, the number of steps in each walk, and the
#' parameters of the logistic distribution (location and scale). The function
...
#' The user can control the number of walks, steps per walk, and the location and scale parameters for the distribution.
...
#' @param .location A numeric value indicating the location parameter of the logistic distribution. Default is 0.
#' @param .scale A numeric value indicating the scale parameter of the logistic distribution. Default is 1.
#' @param .initial_value A numeric value indicating the initial value of the walks. Default is 0.
#' @param .samp A logical value indicating whether to sample the logistic distribution values. Default is TRUE.
...
#'   \item `walk_number`: Factor representing the walk number.
#'   \item `x`: Step index.
#'   \item `y`: Logistic distribution values.
...
#'
#' @export
random_logistic_walk <- function(
  .num_walks = 25, .n = 100, .location = 0, .scale = 1,
  ...
  generate_walk <- function(walk_num) {
    rand_steps <- purrr::map(
      dim_names,
      ~ if (samp) {
        sample(stats::rlogis(n, location = location, scale = scale), size = periods, replace = replace)
      } else {
        stats::rlogis(periods, location = location, scale = scale)
      }
    )
  ...
  attr(res, "fns")           <- "random_logistic_walk"

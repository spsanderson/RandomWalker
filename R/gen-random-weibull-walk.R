...
#' @param .shape Numeric. Shape parameter of the Weibull distribution. Default is 1.
#' @param .scale Numeric. Scale parameter of the Weibull distribution. Default is 1.
...
#' @return A data frame with the random walks and cumulative statistics as columns.
#'
#' @examples
#' set.seed(123)
#' random_weibull_walk()
...
#' }
#'
#' @name random_weibull_walk
NULL

#' @export
#' @rdname random_weibull_walk
random_weibull_walk <- function(
  .num_walks = 25, .n = 100, .shape = 1, .scale = 1,
  ...
  if (.shape < 0) {
    rlang::abort(".shape cannot be less than 0", use_cli_format = TRUE)
  }
  if (.scale < 0) {
    rlang::abort(".scale cannot be less than 0", use_cli_format = TRUE)
  }
  ...
  attr(res, "periods")       <- t
  attr(res, "fns")           <- "random_weibull_walk"

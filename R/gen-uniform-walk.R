...
#'
#' @description
#' The `random_uniform_walk` function generates multiple random walks in 1, 2, or 3 dimensions.
...
#' @param .max A numeric value indicating the maximum of the uniform distribution. Default is 1.
#' @param .initial_value A numeric value indicating the initial value of the walks. Default is 0.
#' @param .samp A logical value indicating whether to sample the uniform distribution values. Default is TRUE.
...
#'
#' @export
random_uniform_walk <- function(.num_walks = 25, .n = 100, .min = 0, .max = 1,
  ...
  if (.min > .max) {
    rlang::abort(".min cannot be greater than .max", use_cli_format = TRUE)
  }
  ...
  attr(res, "min")           <- min_val
  attr(res, "max")           <- max_val

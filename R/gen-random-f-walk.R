...
#' @param .df1 Degrees of freedom 1 for the F distribution. Default is 5.
#' @param .df2 Degrees of freedom 2 for the F distribution. Default is 5.
#' @param .ncp Non-centrality parameter. Default is NULL (central F).
#' @param .initial_value A numeric value indicating the initial value of the walks. Default is 0.
#' @param .samp A logical value indicating whether to sample the F distribution values. Default is TRUE.
...
#'
#' @export
random_f_walk <- function(
  .num_walks = 25, .n = 100, .df1 = 5, .df2 = 5, .ncp = NULL,
  ...
  if (.df1 <= 0 || .df2 <= 0) {
    rlang::abort(".df1 and .df2 must be greater than 0", use_cli_format = TRUE)
  }

...
vignette: >
  %\VignetteIndexEntry{Multi-Dimensional Walks}
...
```{r distribution_examples, eval=FALSE}
# 2D Brownian motion
...
```{r basic_2d_plot, fig.alt="2D random walk trajectories showing multiple walks with start points marked as white circles and end points as red triangles"}
library(ggplot2)
library(dplyr)
...
```{r step_numbers_2d, fig.alt="Single 2D random walk with step numbers labeled at intervals of 10 steps, colored by step progression"}
...
```{r heatmap_2d, fig.alt="Heat map showing the density of 100 random walks with 500 steps each, using a plasma color scale to indicate visit frequency"}
library(ggplot2)
...
```{r 3d_projections, fig.width=10, fig.height=8, fig.alt="Three projection plots showing X-Y, X-Z, and Y-Z views of 3D random walks, arranged with X-Y on top and X-Z and Y-Z side by side below"}
library(ggplot2)
library(patchwork)
...
```{r distance_2d, eval=FALSE, fig.alt="Line plot showing distance from origin over time for 10 random walks in 2D, with a dashed black line showing the theoretical expectation"}
library(dplyr)
...
```{r radial_dist, eval=FALSE, fig.alt="Histogram with density overlay showing the radial distribution of final positions for 500 random walks"}
library(dplyr)
library(ggplot2)
...
```{r convex_hull, eval=FALSE, fig.alt="2D random walk paths with a black polygon showing the convex hull of all final positions"}
library(dplyr)
library(ggplot2)
...
```{r particle_diffusion, eval=FALSE, fig.alt="Line plot showing particle diffusion distance from origin over time, with individual particle paths in light colors and mean distance in bold red"}
...
```{r animal_movement, eval=FALSE, fig.alt="2D random walk showing animal foraging behavior with path colored by distance from home, and a red triangle marking the home location"}
...
```{r boundaries, eval=FALSE, fig.alt="2D random walk with reflecting boundaries shown by a black rectangle, demonstrating how walks bounce back when reaching the boundary"}
...
- **Visualization Guide** - More visualization techniques
- **Use Cases and Examples** - More real-world applications
- **API Reference** - Complete function documentation

---

**Need more examples?** Check out the package documentation and vignettes for additional use cases and applications!
